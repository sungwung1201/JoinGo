...
SCREW_OFFSET_Y = +0.0   # Y축 오프셋 (mm): 양수 -> 로봇 왼쪽, 음수 -> 로봇 오른쪽
...
class RobotController(Node):
    ...
    def robot_control(self):
        ...
        # --- 2. 개별 번호 호출: current_workspace의 나사 좌표로 이동 ---
        ...
        if marker_key:
            ...
            if marker_key in ws_positions:
                ...
                if prep_pos:
                    ...
                    movejx(prep_pos, vel=VELOCITY, acc=ACC, sol=0)
                    mwait()
                else:
        ...
        # --- 3. 워크스페이스 준비 및 각 나사 이동 기능 ---
        elif is_workspace_command:
        ...
        # --- 4. 인식 불가 명령 ---
            ...
            self.get_logger().warn(
                ...
                "'all_check' 또는 '[workspace N] [번호]번 나사 조여' 형태로 말씀해 주세요."
            )
            return False
    ...
    def grip_and_tighten(self, target_pos, approach_height=150.0):
        ...
        # target_pos[3:6] = (rx, ry, rz) 로 회전행렬 복원
