...
class GetKeyword(Node):
    def __init__(self):
       ...
       당신은 사용자의 문장에서 로봇이 수행할 '명령(작업)'과 대상이 되는 '위치(나사 번호)'를 추출해야 합니다.
            ...
            <명령 리스트>
            - all_check : 전체 조사 (모든 나사가 다 보이는 위치로 이동해서 전체 불량 검사)
            - vision_check : 위치 지정과 함께 쓰이며, 특정 위치에서 나사를 자세히 보는 명령
            - torque_check : 특정 위치의 나사를 드라이버로 조여보고 확인하는 명령

            <위치(나사 번호) 리스트>
            - pos1, pos2, pos3, pos4, ... (n번 나사는 posn으로 변환)
 
            <출력 형식>
            - 다음 형식을 반드시 따르세요: [명령1 명령2 ... / 위치1 위치2 ...]
            - 명령과 위치는 각각 공백으로 구분합니다.
            - 명령이 없으면 '/' 앞쪽은 공백 없이 비우고, 위치가 없으면 '/' 뒤는 공백 없이 비웁니다.
            - 다수의 명령과 위치가 동시에 등장할 경우 각각에 대해 정확히 매칭하여 순서대로 출력하세요.

            <특수 규칙>
            - 명확한 명령 명칭이 없지만 문맥상 유추 가능한 경우 리스트 내 항목으로 최대한 추론해 반환하세요.
              예: "조여봐", "단단히 고정해" → torque_check
              예: "자세히 봐", "문제 없는지 봐" (특정 나사 언급 시) → vision_check
              예: "다 검사해", "전체 훑어봐" → all_check
            ...
            - 입력: "1번 나사 자세히 확인해"
            출력: vision_check / pos1

            - 입력: "3번 나사 꽉 조여봐"
            출력: torque_check / pos3

            - 입력: "pos2 시각 검사하고 pos4 토크 체크해"
            출력: vision_check torque_check / pos2 pos4

            - 입력: "2번이랑 3번 나사 조여줘"
            출력: torque_check torque_check / pos2 pos3
    ...
    def extract_keyword(self, output_message):
        ...
        if "/" in result:
            object_str, target_str = result.strip().split("/", 1)
        else:
            object_str, target_str = result.strip(), ""

        # 여기서 object_str과 target_str을 split 해야 합니다!
        ...
        # 가장 중요한 부분! 명령과 위치를 모두 합쳐서 반환해야 합니다.
